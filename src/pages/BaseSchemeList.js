...
const BaseSchemeList = () => {
    ...
    useEffect(() => {
        const fetchSchemes = async () => {
            ...
            try {
                ...
                const response = await fetch(`${url}/api/base/schemes/getAllSchemes`, {
                    headers: {
                ...
                if (data.success) {
                    ...
                    const formattedSchemes = data.data.map(scheme => {
                        ...
                        return {
                            ...
                            // Add distributorType field
                            distributorType: scheme.distributorType || 'individual'
                        };
                    });
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => {
                ...
                if (record.distributorType === 'group') {
                    // For group type, show the group codes directly
                    return (
                        <div style={tableStyles.distributorContainer}>
                            {record.distributors.map((groupCode, index) => (
                                <div key={index}>
                                    {typeof groupCode === 'string' ? groupCode :
                                        (groupCode.CUSTOMERGROUPID || `Group ${index + 1}`)}
                                </div>
                            ))}
                        </div>
                    );
                } else {
                    // For individual type, show customer accounts
                    return (
                        <div style={tableStyles.distributorContainer}>
                            {record.distributors.map((dist, index) => (
                                <div key={index}>
    ...
    const handleExportByDate = async () => {
        ...
        try {
            ...
            const response = await fetch(
                `${url}/api/base/schemes/exportByDate?startDate=${startDate}&endDate=${endDate}&format=excel`,
                {
                    method: 'GET',
                    headers: {
                        'Authorization': `Bearer ${token}`
    ...
    const handleDeleteScheme = async () => {
        ...
        try {
            ...
            const response = await fetch(`${url}/api/base/schemes/delete/${schemeToDelete}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
    ...
    return (
        ...
        <Row justify="space-between" align="middle">
          ...
          <Col>
            ...
            <Card style={{ marginBottom: '16px' }}>
                <Title level={4}>Base Scheme</Title>
